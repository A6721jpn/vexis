...
### 4. GUIメッシュ生成専用モードの実装 (v1.4.0)
- **目的:** ユーザーが解析時間をかけずにメッシュ品質のみを素早く確認できるようにするため。
- **実装内容:**
    - **モデル拡張:** `JobStatus` に `MESH_GENERATED` ステータスを追加。
    - **バックエンドロジック:** `JobManager` および `AnalysisWorker` に `mesh_only` フラグを導入し、メッシュ生成後に処理を早期終了するロジックを実装。
    - **UI改修:** 
        - ツールバーに「Gen Mesh」ボタンを追加（アイコン: `meshgen.svg`）。
        - 「Mesh Gen」完了時に自動的にバッチ状態をリセットし、即時の再操作を可能にするシグナル処理（`batch_finished`）を追加。
        - 完了ジョブ選択時に `MeshPreview` パネルへ自動切り替え、3Dビューワーでマウス操作可能なメッシュプレビューを表示。
    - **ブランチ管理:** 機能実装を `V140` ブランチに集約し、`main` へマージ可能な状態へ整備。

### 5. コンフィグ外部化の推進
- **変更:** `RUBBER_SELF_CONTACT` のペナルティ係数 (`contact_penalty`) を `config.yaml` の `analysis` セクションに外部化。
- **効果:** メッシュ自己交差の解消などのチューニングを、テンプレートファイル (`.feb`) を直接編集せずに行えるようになった。

### 6. バージョンアップデート (v1.4.0)
- **変更:** `config.yaml`, `version.py`, `release_notes.md` を更新し、バージョンを `1.4.0` に引き上げ。
- **リリースノート:** メッシュ生成モードの新設とGUI改善（ボタン配置、自動リセット）を記載。

## 次回のタスク
- [ ] 修正版のGUIによる実機解析でのパラメータ反映確認。
- [ ] 1.4.0正式版のリリースと配布。
- [ ] 複数ケースでのメッシュ生成モードの実地検証。
