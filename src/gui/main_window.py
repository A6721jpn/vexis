...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.solver_status_label.setStyleSheet(
            "color: #6F8098; font-size: 12px; padding-right: 5px;"
        )
    ...
    def _setup_toolbar(self):
        ...
        for item in actions:
            if item is None:
                ...
                action = QAction(
                    load_icon(icon_name, fallback, self.style()), label, self
                )
        ...
        self._sleep_icon_on = load_icon(
            "eye-solid", QStyle.SP_DialogApplyButton, self.style()
        )
        self._sleep_icon_off = load_icon(
            "eye-closed", QStyle.SP_DialogCancelButton, self.style()
        )
        ...
        self.gen_mesh_action = QAction(
            load_icon("meshgen", QStyle.SP_FileDialogListView, self.style()),
            "Gen Mesh",
            self,
        )
    ...
    def _on_job_status_changed(self, job_id, status):
        ...
        if status == JobStatus.ERROR:
            ...
            QMessageBox.critical(
                self, "Analysis Error", f"Job '{job.name}' Failed.\n\nError: {err_msg}"
            )
        elif status == JobStatus.STOPPED:
            job = self.jobs[job_id]
            QMessageBox.information(
                self, "Analysis Stopped", f"Job '{job.name}' was stopped by user."
            )
    ...
    def _on_job_log_added(self, job_id, line):
        if (
            self._get_current_job_id() == job_id
        ):  # Corrected from 'current_job_id' to 'self._get_current_job_id()'
    ...
    def _update_solver_status(self):
        ...
        if is_found:
            ...
            self.solver_status_label.setStyleSheet(
                "color: #6F8098; font-size: 12px; padding-right: 10px;"
            )
        else:
            self.solver_status_label.setText(status_text)
            self.solver_status_label.setStyleSheet(
                "color: #FFD700; font-size: 12px; padding-right: 10px; font-weight: bold;"
            )
