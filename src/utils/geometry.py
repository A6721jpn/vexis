...
def get_relative_coordinates(
    node: np.ndarray, bbox: Tuple[np.ndarray, np.ndarray]
) -> np.ndarray:
    """
...
def get_absolute_coordinates(
    relative_coords: np.ndarray, bbox: Tuple[np.ndarray, np.ndarray]
) -> np.ndarray:
    """
...
def query_kdtree_distance(
    kdtree: KDTree, points: np.ndarray, distance_upper_bound: float = np.inf
...
def filter_nodes_by_relative_bounds(
    nodes: np.ndarray,
    relative_bounds: Tuple,
    global_bbox: Tuple[np.ndarray, np.ndarray],
) -> np.ndarray:
    """
...
def extract_boundary_faces(elements: List) -> List[Tuple]:
    ...
    if num_nodes == 8:
        ...
        print(
            f"[WARN] extract_boundary_faces: Unsupported element type with {num_nodes} nodes"
        )
        return []
