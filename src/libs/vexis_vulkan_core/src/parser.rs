...
impl XpltFastParser {
            ...
            for dict_section in &dict_block.children {
                ...
                if is_node || is_domain {
                    ...
                    for item in &dict_section.children {
                        if item.tag == 0x01020001 {
                            // Dictionary item
                            let mut name = String::new();
                            let mut type_id = 0;
                            let mut format_id = 0;
                            for prop in &item.children {
                                if prop.tag == 0x01020002 {
                                    type_id = Self::read_u32(&mmap, prop.offset, endian);
                                }
                                if prop.tag == 0x01020003 {
                                    format_id = Self::read_u32(&mmap, prop.offset, endian);
                                }
                                if prop.tag == 0x01020004 {
                                    ...
                                    name = String::from_utf8_lossy(&mmap[prop.offset..end])
                                        .into_owned();
                                }
                            }
                            if is_node {
                                node_vars.push(VarInfo {
                                    name,
                                    type_id,
                                    format_id,
                                    index: var_id_counter,
                                });
                            } else {
                                domain_vars.push(VarInfo {
                                    name,
                                    type_id,
                                    format_id,
                                    index: var_id_counter,
                ...
                for i in 0..num_floats {
                    if ptr + i * 4 + 4 <= end {
                        let f = match self.endian {
                            Endianness::Little => {
                                LittleEndian::read_f32(&self.mmap[ptr + i * 4..ptr + i * 4 + 4])
                            }
                            Endianness::Big => {
                                BigEndian::read_f32(&self.mmap[ptr + i * 4..ptr + i * 4 + 4])
                            }
                        };
                        values_flat.push(f);
                    }
                    ...
                    for i in 0..nodes_in_region {
                        if ptr + i * 12 + 12 <= end {
                            let x = match self.endian {
                                Endianness::Little => LittleEndian::read_f32(
                                    &self.mmap[ptr + i * 12..ptr + i * 12 + 4],
                                ),
                                Endianness::Big => {
                                    BigEndian::read_f32(&self.mmap[ptr + i * 12..ptr + i * 12 + 4])
                                }
                            };
                            let y = match self.endian {
                                Endianness::Little => LittleEndian::read_f32(
                                    &self.mmap[ptr + i * 12 + 4..ptr + i * 12 + 8],
                                ),
                                Endianness::Big => BigEndian::read_f32(
                                    &self.mmap[ptr + i * 12 + 4..ptr + i * 12 + 8],
                                ),
                            };
                            let z = match self.endian {
                                Endianness::Little => LittleEndian::read_f32(
                                    &self.mmap[ptr + i * 12 + 8..ptr + i * 12 + 12],
                                ),
                                Endianness::Big => BigEndian::read_f32(
                                    &self.mmap[ptr + i * 12 + 8..ptr + i * 12 + 12],
                                ),
                    ...
                    for i in 0..items_in_region {
                        if ptr + i * 4 * comp_count + 4 <= end {
                            let f = match self.endian {
                                Endianness::Little => LittleEndian::read_f32(
                                    &self.mmap
                                        [ptr + i * 4 * comp_count..ptr + i * 4 * comp_count + 4],
                                ),
                                Endianness::Big => BigEndian::read_f32(
                                    &self.mmap
                                        [ptr + i * 4 * comp_count..ptr + i * 4 * comp_count + 4],
    ...
    pub fn get_base_coordinates<'py>(
        &self,
        py: Python<'py>,
    ) -> PyResult<Bound<'py, pyo3::types::PyAny>> {
        ...
        let coords_block = Self::find_block(&root_blocks, 0x01041001)
            .or_else(|| Self::find_block(&root_blocks, 0x01041200));
        ...
        if let Some(block) = coords_block {
            ...
            let (is_v2, item_size) = if block.tag == 0x01041200 {
                (true, 16)
            } else {
                (false, 12)
            };
            ...
            Err(PyValueError::new_err(
                "Node coordinates block (0x01041001/0x01041200) not found",
            ))
        }
    }
    ...
    pub fn get_domain_elements<'py>(
        ...
        if domain_idx >= domains.len() {
            return Err(PyValueError::new_err(format!(
                "Invalid domain index. Found {} domains.",
                domains.len()
            )));
        }
        ...
        for eb in &element_blocks {
            ...
            for i in 0..num_ints {
                let v = match self.endian {
                    Endianness::Little => {
                        LittleEndian::read_u32(&self.mmap[offset + i * 4..offset + i * 4 + 4])
                    }
                    Endianness::Big => {
                        BigEndian::read_u32(&self.mmap[offset + i * 4..offset + i * 4 + 4])
                    }
                };
                all_ints.push(v);
            }
        ...
        if !all_ints.is_empty() {
            ...
            Err(PyValueError::new_err(
                "Elements (0x01042201) not found in domain",
            ))
        }
    }
}
...
impl XpltFastParser {
    ...
    fn find_block(blocks: &[Block], target_tag: u32) -> Option<&Block> {
        for b in blocks {
            if b.tag == target_tag {
                return Some(b);
            }
    ...
    fn is_branch(tag: u32) -> bool {
        let branch_tags = [
            0x01000000, 0x01010000, 0x01020000, 0x01021000, 0x01022000, 0x01023000, 0x01024000,
            0x01025000, 0x01020001, 0x01030000, 0x01030001, 0x01040000, 0x01041000, 0x01041100,
            0x01042000, 0x01042100, 0x01042101, 0x01042200, 0x01043000, 0x01043100, 0x01043101,
            0x01043200, 0x01044000, 0x01044100, 0x01045000, 0x01045100, 0x01050000, 0x02000000,
            0x02010000, 0x02020000, 0x02020300, 0x02020400, 0x02020001,
        ];
    ...
    fn parse_blocks(
        ...
        while offset + 8 <= end_limit && offset + 8 <= data.len() {
            ...
            if Self::is_branch(tag) && depth < 10 {
                children =
                    Self::parse_blocks(data, offset + 8, offset + 8 + size, endian, depth + 1);
            }
