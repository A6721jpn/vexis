...
class SetReconstructor:
        ...
        for surface in self.mesh_section.findall("Surface"):
            ...
            if partner_name:
                ...
                if self._is_surface_on_this_part(partner_name):
                    print(
                        f"  [Refining] Set '{name}' has partner '{partner_name}' on SAME part -> Forcing Strategy A"
                    )
                    strategy = "A"
                elif p_nodes is not None and len(p_nodes) > 0:
                    # Calculate centroids for partner faces
                    p_centroids = geometry_utils.calculate_face_centroids(
                        p_nodes, p_faces
                    )
            ...
            if strategy == "A":
                ...
                def_dict["relative_bounds"] = self._calculate_relative_bounds_for_check(
                    nodes_array
                )
            else:
    ...
    def reconstruct(self, new_nodes, new_elements_connectivity):
        ...
        new_boundary_faces = geometry_utils.extract_boundary_faces(
            new_elements_connectivity
        )
        new_face_centroids = geometry_utils.calculate_face_centroids(
            new_nodes, new_boundary_faces
        )

        print(f"[DEBUG] reconstruct: new_bbox={new_bbox}")
        print(f"[DEBUG] reconstruct: found {len(new_boundary_faces)} boundary faces")
        print(
            f"[DEBUG] reconstruct: calculated centroids shape={new_face_centroids.shape}"
        )
        ...
        for d in self.set_definitions:
            ...
            if d["type"] == "NodeSet":
                ...
                if strategy:
                    ...
                    print(
                        f"[WARN] Unknown strategy '{strategy_key}' for surface '{name}'"
                    )
