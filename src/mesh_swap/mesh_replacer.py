...
def replace_mesh(tree, new_nodes, new_elements, part_name, elem_type="hex8"):
    ...
    try:
        ...
        print(
            f"SetReconstructor initialized for {part_name}. Found {len(reconstructor.set_definitions)} sets to preserve."
        )
    ...
    if reconstructor:
        ...
        reconstructor.set_definitions = [
            d for d in reconstructor.set_definitions if d["name"] != target
        ]
        ...
        reconstructor.set_definitions = [
            d for d in reconstructor.set_definitions if d["name"] != target_top
        ]
    ...
    if target_elems_node is None:
        ...
        for elems_node in mesh_section.findall("Elements"):
            ...
            if is_match:
                target_elems_node = elems_node
                print(
                    f"Found related Element block: name='{elems_node.get('name')}', type='{elems_node.get('type')}'"
                )
                break
    ...
    print(
        f"Renumbering: Nodes start at {start_node_id}, Elements start at {start_elem_id}"
    )
    ...
    if reconstructor:
        ...
        try:
            ...
            print(
                f"Reconstructed {len(reconstructed_data.get('NodeSet', {}))} NodeSets and {len(reconstructed_data.get('Surface', {}))} Surfaces."
            )
        ...
        for child in sp:
            if child.tag in ["primary", "secondary"]:
                if child.text and child.text.strip() in removed_set_names:
                    should_remove = True
                    print(
                        f"SurfacePair '{sp.get('name')}' references removed surface '{child.text.strip()}'"
                    )
                    break
        ...
        for child in element:
            ...
            for attrib in ["node_set", "surface", "elem_set", "pair", "surface_pair"]:
                ...
                if is_invalid_ref(val):
                    ...
                    print(
                        f"Removing invalidated item (attribute ref): tag='{child.tag}', name='{child.get('name')}', invalid_ref='{val}'"
                    )
                    break
                ...
                for sub in child:
                    if sub.text and is_invalid_ref(sub.text.strip()):
                        if sub.tag in [
                            ...
                            should_remove_child = True
                            print(
                                f"Removing invalidated item (child text ref): tag='{child.tag}', name='{child.get('name')}', invalid_ref='{sub.text.strip()}'"
                            )
                            break
...
def save_file(tree, output_path):
    ...
    tree.write(
        output_path, encoding="ISO-8859-1", xml_declaration=True, pretty_print=False
    )
...
def adjust_keycap_height(tree, rubber_max_z, ref_z=2.65):
    ...
    print(
        f"Adjusting KEYCAP Z-height: Shift = {shift_val:.6f} (Rubber Top: {rubber_max_z:.6f}, Ref: {ref_z})"
    )
    ...
    for node in keycap_nodes.findall("node"):
        if node.text:
            try:
                coords = [
                    float(x) for x in node.text.strip().replace("\t", "").split(",")
                ]
                # Shift Z
                coords[2] += shift_val
                # Write back
