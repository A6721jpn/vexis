        ...
        with open(log_path, "a", encoding="utf-8") as f_log:
            ...
            try:
                ...
                if proc.stdout is None:
                    raise RuntimeError("Meshing subprocess did not expose stdout.")
...
def run_integration(
    ...
    template,
    out_feb,
    push_dist_override=None,
    steps=None,
    material_name=None,
    material_config_path=None,
    contact_penalty=None,
    log_path=None,
):
    from src.mesh_swap.mesh_replacer import (
        adjust_keycap_height,
        load_reference,
        override_control_params,
        override_rigid_bc,
        replace_mesh,
        save_file,
    )
    ...
    with redirect_output_to_file(log_path):
        ...
        nodes_mapping, rubber_max_z = replace_mesh(
            tree, points, cells_data, "RUBBER_OBJ", cell_type_str
        )
...
def run_solver_and_extract(
    ...
    if not valid_candidates and os.path.exists(
        system_path
    ):  # If all others are gone, but system path exists
        valid_candidates = [system_path]
    elif (
        not valid_candidates
    ):  # If nothing exists, just try the bundled path as a default
    ...
    try:
        ...
        f_global = (
            open(log_path, "a", encoding="utf-8") if log_path else open(os.devnull, "w")
        )
        ...
        try:
            ...
            for current_exe in valid_candidates:
                ...
                if exe_dir not in current_env.get("PATH", ""):
                    current_env["PATH"] = (
                        exe_dir + os.pathsep + current_env.get("PATH", "")
                    )
                ...
                try:
                    ...
                    if proc.stdout is None:
                        raise RuntimeError("Solver subprocess did not expose stdout.")
                    ...
                    for line in proc.stdout:
                        ...
                        if "time" in line:
                            match = re.search(
                                r"time\s*=\s*([\d\.eE\+\-]+)", line, re.IGNORECASE
                            )
                            if match:
                                try:
                    ...
                    f_global.write(
                        f"DEBUG: Solver Finished with Return Code = {last_error_code}\n"
                    )
                    f_global.flush()
    ...
    with redirect_output_to_file(log_path):
        ...
        if os.path.exists(target_data_path):
            ...
            safe_move(
                os.path.join(result_dir, "force_displacement.csv"),
                os.path.join(result_dir, f"{base_name}_result.csv"),
            )
            safe_move(
                os.path.join(result_dir, "force_displacement.png"),
                os.path.join(result_dir, f"{base_name}_graph.png"),
            )
...
def update_material_params(tree, material_name, yaml_path):
    ...
    if "density" in params:
        set_val(target_mat, "density", params["density"])
    if "k" in params:
        set_val(target_mat, "k", params["k"])
    if "pressure_model" in params:
        set_val(target_mat, "pressure_model", params["pressure_model"])
